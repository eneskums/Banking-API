...
from .models import Customer, Account, Transfer, Log
...
def create_account(request):
    ...
    try:
        ...
        account = Account(customer=customer, balance=initial_balance)
        account.save()
    ...
    try:
        with transaction.atomic():
            ...
            if sender_account_id == receiver_account_id:
                return Response({'error': 'Accounts are not same'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            transfer = Transfer(source_account=sender_account, destination_account=receiver_account,
            ...
            return Response({'transfer': transfer_serializer.data, 'sender': sender_serializer.data,
...
def transfer_history(request, account_id):
    ...
    try:
        account = Account.objects.get(pk=account_id)
        query = Q(source_account=account) | Q(destination_account=account)
        if start_date:
